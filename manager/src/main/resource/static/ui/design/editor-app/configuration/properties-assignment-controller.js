...
　　　　		items+='<span>'+name+'</span>'
	...
	selUser = $scope.popup.assignmentObject.idm.candidateUsers;
	setTimeout(function(){
		for(var i = 0;i < selUser.length;i++){
			var name = selUser[i].full_name;
			var items='<li>'
	　　　　	items+='<span>'+name+'</span>'
			items+='<div class="actions">'
			items+='<a onclick="removeUsers(\''+selUser[i].id+'\')">'
			items+='<span class="undis">'+selUser[i]+'</span>'
	...
	selGroups = $scope.popup.assignmentObject.idm.candidateGroups;
	setTimeout(function(){
...
//              }
                if(selUserOne.length > 0){
					idm.assignee = selUserOne[0];
					$scope.popup.assignmentObject.idm.assignee = selUserOne[0];
				}
...
//              }
				if(selUser.length > 0){
					idm.candidateUsers = selUser;
					$scope.popup.assignmentObject.idm.candidateUsers = selUser;
					console.log(selUser)
...
//              }
				...
				if(selGroups.length > 0){
					idm.candidateGroups = selGroups;
					$scope.popup.assignmentObject.idm.candidateGroups = selGroups;
				}
            }
    ...
    $scope.openUserDialog = function() {
    	jQuery(".userSelect").removeClass("undis");
    	$scope.userSelectDialog = false;
    	$scope.userSelectDialog = true;
    	if ("user" == $scope.assignmentOption.id) {
    		userType = "user";
        } else if ("users" == $scope.assignmentOption.id) {
        	userType = "users";
   ...
   $scope.openRoleDialog = function() {
    	jQuery(".roleSelect").removeClass("undis");
    	$scope.roleSelectDialog = false;
    	$scope.roleSelectDialog = true;
    };
