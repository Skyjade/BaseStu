package com.sky.nettyServer.handler;
...
public class ServerHandler  extends SimpleChannelInboundHandler<String> {
    ...
    protected void channelRead0(ChannelHandlerContext context, String s)
        ...
        for (Channel ch : group) {
            if (ch == channel) {
                ch.writeAndFlush(new Message("[you]: " + s + "\n"));
            } else {
                ch.writeAndFlush(new Message("[" + channel.remoteAddress() + "]: " + s + "\n"));
            }
        }
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ...
        for (Channel ch : group) {
            if (ch == channel) {
                ch.writeAndFlush(new Message("[" + channel.remoteAddress() + "] coming"));
            }
        }
        group.add(channel);
    }
    ...
    public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
        ...
        for (Channel ch : group) {
            if (ch == channel) {
                ch.writeAndFlush(new Message("[" + channel.remoteAddress() + "] leaving"));
            }
        }
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        ctx.writeAndFlush(new Message("[server]: 欢迎"));
